...
| `app.py` (`/api/internal/summary/live` GET, `_summary_live_html()`) + `weekly_summary.summary_live_data()` + `templates/_summary_live.html` | Blocco **«Stato azienda» live** in cima alla Dashboard (`templates/dashboard.html`): card di **KPI orizzontali** che replica lo stile del tile calendario (`eventi_settimana_tile`: header-label + colonne `divide-x`), non il testo completo del messaggio Telegram. Quattro colonne, numeri grandi — 📅 demo prenotate (sotto il numero, i **nomi** di chi ha prenotato), 🚀 nuovi clienti, 💻 **numero** di PR integrate (senza titoli), 💶 ricavi del **solo mese corrente** con la **proiezione** come valore principale e, sotto in piccolo, il **reale a oggi** (maturato). Demo e nuovi clienti a **zero** → numero **ambra** con warning ⚠. `summary_live_data()` torna il dict (variante settimanale, riusa gli helper di §9), reso col partial `_summary_live.html`. La home lo carica **lazy** (fetch JS). Cache in-process **TTL 10 min** (`_summary_live_cache`) perché serve una chiamata di rete a GitHub; su errore degrada all'ultima versione buona, e se non c'è ancora nulla il client nasconde il blocco. Sotto «PR integrate» un pulsante **Riassunto** apre nel modale (`/development-summary` in iframe) il **resoconto AI sviluppo** (vedi sotto). |
| `app.py` (`/development-summary` GET, `_get_development_summary()`, tabella `development_summary_cache`) + `templates/development_summary.html` | Riassunto AI sviluppo **persistito**: la modale «Riassunto» non rigenera ad ogni apertura. La firma è l'**insieme dei numeri PR** nella finestra (`_dev_summary_signature`); finché non cambia si riusa il markdown salvato, quando cambia (**nuova PR** integrata, o PR uscita dalla finestra) si rigenera via `development_summary.build_development_markdown` (LLM via gateway OneCLI, §9-bis). `?force=1` forza. Se GitHub è giù e c'è cache, la serve stantia invece di rigenerare un errore. Riga singola `id=1`. |
...
- **🚀 Distribution** — riga `📅 Demo prenotate: **N**` (meeting cal.com **prenotati** nella finestra) + tabella `Cliente | Data` dei nuovi clienti paganti nella finestra: `stripe_subscriptions` `active`/`trialing`, `is_error=0`. Zero clienti → `⚠️ **Nessun cliente onboardato**`. Il conteggio demo (`weekly_summary._demo_prenotazioni` / `_demo_prenotate`) usa `gcal_events` con `cal_booking_id` non nullo, `created` (data di **prenotazione**, non di svolgimento) nella finestra, escludendo gli annullati (`status='CANCELLED'`). **Solo le vere demo di vendita**: filtra l'event type cal.com `Demo Autofatturiamo — …` (prefisso `_DEMO_TYPE_PREFIX`), escludendo gli altri tipi di booking (Assistenza, Attivazione, Prova Gratuita, Meeting, Check).
